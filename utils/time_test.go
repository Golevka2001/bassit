...
func TestGetVibDuration(t *testing.T) {
	...
	for i := range 100 {
		...
		assert.GreaterOrEqual(t, duration, config.StringVibDurMin*time.Millisecond, "Duration should be at least %d ms", config.StringVibDurMin)
		assert.LessOrEqual(t, duration, config.StringVibDurMax*time.Millisecond, "Duration should be at most %d ms", config.StringVibDurMax)
	}
}
